...
- **Multi-Pass Pumpkin Verification (`pumpkin_primary_scan`):** Implements a multi-pass transactional verification loop ($PUMPKIN\_PASSES$). The script scans the field, replaces dead or incorrect entities, and blocks global harvest execution until a complete readiness convergence state (`ready[0] == True`) is achieved.
