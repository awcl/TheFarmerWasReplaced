	...
	if measure():
		...
		if not ahead and not behind:
			...
			for _ in range(body_columns):
				...
				directions = get_directions(pos, size)
				ahead, _, x_bound, y_bounds, y_next = get_boundaries(
					pos, target, size, directions
				)
				if ahead:
					break
...
def get_data():
	crops = [Entities.Grass, Entities.Bush, Entities.Tree, Entities.Carrot]
	resources = {Entities.Grass: Items.Hay, Entities.Bush: Items.Wood, Entities.Tree: Items.Wood, Entities.Carrot: Items.Carrot}
...
def harvest_grid(crop, laps=get_world_size(), limit=0, fertilize=False):
	quick_pos(0, 0)
	size = get_world_size()
	polyculture = num_unlocked(Unlocks.Polyculture) > 0
	if not polyculture:
		basic_harvest(size, crop, fertilize)
		return
	if laps == 0:
		laps = None
	for x in range(size):
		pos = (x, 0)
		if not spawn_drone(harvest_column, pos, size, crop, laps, limit, fertilize):
			harvest_column(pos, size, crop, laps, limit, fertilize)

def harvest_column(pos, size, crop, laps, limit, fertilize):
	crops, resources = get_data()
	crops.remove(crop)
	resource = resources[crop]
	companions = {}
	water_level = 0.7
	x, y = pos
	quick_pos(x, y)
	while laps == None or laps > 0:
		viable = True
		for y in range(size):
			next_crop = None
			viable = (x + y) % 2 == 0
			if viable:
				...
				if valid_companion(companion):
					plant_type, (c_x, c_y) = companion
					if (c_x, c_y) not in companions:
						companions[(c_x, c_y)] = plant_type
			else:
				entity = get_entity_type()
				if entity not in crops:
					idx = (x + y) % 3
					next_crop = crops[idx]
				if (x, y) in companions:
					next_crop = companions[(x, y)]
					companions.pop((x, y))
				if next_crop and not is_entity(next_crop):
					harvest()
					plant_(next_crop)
			if limit and num_items(resource) >= limit:
				return
			move(North)
		if laps:
			laps -= 1

def basic_harvest(size, crop, fertilize):
	available = [Entities.Grass]
	unlocks = {Unlocks.Plant: Entities.Bush, Unlocks.Trees: Entities.Tree, Unlocks.Carrots: Entities.Carrot}
	for unlock in unlocks:
		if num_unlocked(unlock) > 0:
			available.append(unlocks[unlock])
	alternates = []
	for entity in available:
		if entity != crop:
			alternates.append(entity)
	def basic_column(size, crop, fertilize):
		for _ in range(size):
			x, y = get_pos_x(), get_pos_y()
			viable = True
			if alternates:
			...
			if viable:
				...
				idx = (x + y) % len(alternates)
				alternate = alternates[idx]
				harvest_()
				plant_(alternate)
			move(North)
		if fertilize:
			for _ in range(size):
				...
				move(North)
	for _ in range(size):
		if not spawn_drone(basic_column, size, crop, fertilize):
			basic_column(size, crop, fertilize)
		move(East)
...
def plant_(crop):
	...
	if crop and not is_entity(crop):
		till_(should_till[crop])
		return plant(crop)
	return True

def replant(crop):
	if is_entity(crop) and harvest_():
		plant_(crop)

def till_(wanted):
	if not is_soil() and wanted:
		till()
	if is_soil() and not wanted:
		till()
...
def valid_companion(companion):
	if not companion:
		till()
		return False
	_, (x, y) = companion
	return x == get_pos_x() and (x + y) % 2 != 0

def water_(level=0.5, repeat=False):
	def can_water():
		has_water = num_items(Items.Water) >= 1
		low_water = get_water() < level
		return has_water and low_water
	if can_water():
